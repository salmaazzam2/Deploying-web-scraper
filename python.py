...
def extract(soup):
    ...
    for result in search_results:
        ...
        for loc in location_array:
            if loc.istitle() and loc.lower() not in remote:
                desired_loc = loc
                break
...
def fetch_phone_number(company_name, location):
    ...
    try:
        ...
        time.sleep(2)
        try:
            phone_number_div = driver.find_element(By.CLASS_NAME, "pwimAb")
...
def data():
    try:
        ...
        pages = request.args.get('pages', default=1, type=int)  # Default value is 1
        start = request.args.get('start', default=1, type=int)  # Default value is 1
